...
#include "intStack.h" // Include the header file for the stack functions
...
// Static globals for stack implementation
static int top = 0;       // Tracks the top of the stack (initialized to 0)
static int stack[100];    // Array to hold the stack elements (size = 100)
...
int pop() {
    ...
    return stack[--top]; // Decrement top and return the element at the new top
}
...
void push(int thing2push) {
    if (top >= 100) { // Check if the stack is full
        fprintf(stderr, "Error: Stack is full.\n"); // Print an error message
        return; // Exit the function without pushing
    }
    stack[top++] = thing2push; // Add the element to the stack and increment top
}
 ...
 */
int isEmpty() {
    return top == 0; // Return 1 if the stack is empty, otherwise return 0
}